...
For this repo there is also a local bootstrap script that creates a workspace-scoped virtualenv on top of an existing Python 3.9 + PyTorch install:
...
By default the script uses `/home/workstation/miniconda3/envs/fusion/bin/python` as the base interpreter, clones `CenterNet2` and `CenterPoint` into `third_party/`, and writes the required `PYTHONPATH` exports into `env.sh`.
It also installs `detectron2` from a local checkout in `third_party/detectron2`; on hosts without a working CUDA runtime it automatically falls back to a CPU-only build.

If you already have a dedicated conda environment with working CUDA support, activate that environment first and then run `source env.sh`. Do not stack `.venv` on top of conda.
...
To evaluate the trained MVP voxel model from the repository root and fail on a
quality regression below mAP `0.60` or NDS `0.65`, run:
...
The test uses the active MVP config and `latest.pth` by default, writes fresh
evaluation artifacts to `eval_latest`, and creates `mvp_test_report.json` with
the complete nuScenes metrics summary, detailed precision/recall data, and
CenterPoint inference latency/throughput. It also writes a thesis-oriented
`summary.md` with per-class and reference comparisons and a long-form
`thesis_metrics.csv` ready for plots or statistical software. Machine-readable
JSON uses `null` for class-inapplicable errors. Timing excludes offline
generation of virtual points from camera images.
Use `--check-only` to validate inputs without inference, `--gpus 4` for
distributed evaluation, or `--metrics-only PATH/metrics_summary.json` to check
an existing nuScenes result.
## MODEL ZOO 
