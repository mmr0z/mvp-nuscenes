...
def is_valid_virtual_points_file(path):
    try:
    ...
    return (
        ...
        and 'virtual_points' in data
        and 'real_points' in data
        and 'real_points_indice' in data
    )


def save_virtual_points_file(path, data):
    tmp_path = path + '.tmp'
...
def validate_device(device):
    if device.startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "Verify that the NVIDIA driver is loaded, the GPU is visible in this shell, "
            "and the current environment has CUDA access before running virtual_gen.py."
        )
    return device
...
class PaintDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.isfile(output_path):
            if is_valid_virtual_points_file(output_path):
                return []
...
@torch.no_grad()
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=50, dist_thresh=3000, num_camera=6, intrinsics=None, transforms=None, device=None):
    ...
    virtual_point_instance_ids = torch.arange(1, 1+masks.shape[0], 
        dtype=torch.float32, device=device).reshape(masks.shape[0], 1, 1).repeat(1, num_virtual, 1)
...
def init_detector(args):
    ...
    if "MODEL.DEVICE" not in args.opts:
        args.opts.extend(["MODEL.DEVICE", args.device])
    cfg = setup(args)
...
@torch.no_grad()
def process_one_frame(info, predictor, data, num_camera=6, device=None):
    device = resolve_device(device)
    ...
    one_hot_labels = [] 
    for i in range(10):
        one_hot_label = torch.zeros(10, device=device, dtype=torch.float32)
        one_hot_label[i] = 1
    ...
    for camera_id in range(num_camera):
        ...
        camera_id = torch.tensor(camera_id, dtype=torch.float32, device=device).reshape(1,1).repeat(pred_mask.shape[0], 1)
    ...
    P = projectionV2(
        to_tensor(lidar_points, device=device),
        to_batch_tensor(all_cams_from_lidar, device=device),
        to_batch_tensor(all_cams_intrinsic, device=device),
        device=device
    )
    camera_ids = torch.arange(6, dtype=torch.float32, device=device).reshape(6, 1, 1).repeat(1, P.shape[1], 1)
    P = torch.cat([P, camera_ids], dim=-1)

    if len(masks) == 0:
        res = None
    else:
        res  = add_virtual_mask(masks, labels, P, to_tensor(lidar_points, device=device), 
            intrinsics=to_batch_tensor(all_cams_intrinsic, device=device), transforms=to_batch_tensor(all_cams_from_lidar, device=device), device=device)
...
def main(args):
    args.device = validate_device(resolve_device(args.device))
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        res = process_one_frame(info, predictor, data, device=args.device)
        ...
        save_virtual_points_file(output_path, data_dict)
