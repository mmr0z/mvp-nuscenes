...
def to_batch_tensor(tensor, device=None, dtype=torch.float32):
    device = resolve_device(device)
    return torch.stack([to_tensor(x, device=device, dtype=dtype) for x in tensor], dim=0)

def batch_view_points(points, view, normalize, device=None):
...
def reverse_view_points(points, depths, view, device=None):
...
@torch.no_grad()
def projectionV2(points, all_cams_from_lidar, all_cams_intrinsic, H=900, W=1600, device=None):
    # projected_points
