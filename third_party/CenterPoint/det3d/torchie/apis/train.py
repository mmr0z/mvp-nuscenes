...
from det3d.torchie.trainer import hooks as trainer_hooks
...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    if cfg.lr_config.type == "one_cycle":
        ...
        cfg.lr_config = None
    elif cfg.lr_config.type == "cosine":
        optimizer = build_optimizer(model, cfg.optimizer)
        t_max = cfg.lr_config.get("T_max", total_steps)
        if cfg.lr_config.get("by_epoch", False):
            t_max = t_max * len(data_loaders[0])
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=t_max,
            eta_min=cfg.lr_config.get("eta_min", 0.0),
        )
        cfg.lr_config = None
    else:
        optimizer = build_optimizer(model, cfg.optimizer)
    ...
    for hook_cfg in cfg.get("custom_hooks", []):
        ...
        trainer.register_hook(obj_from_dict(hook_cfg, trainer_hooks), priority=priority)
